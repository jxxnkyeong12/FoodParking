...
//		Authorization: Bearer ${ACCESS_TOKEN}
	...
	public String naver_callback(String state, HttpSession session, String code, String error) {
//		Callback 응답 정보
//		API 요청 성공시 : http://콜백URL/redirect?code={code값}&state={state값}
//		API 요청 실패시 : http://콜백URL/redirect?state={state값}&error={에러코드값}&error_description={에러메시지}
		if (error != null || !state.equals((String) session.getAttribute("state")))
			return "redirect:/";
		...
		if (json.getString("resultcode").equals("00")) {
			...
			vo.setSocial("N");
			vo.setEmail(json.getString("email"));
			vo.setName(json.has("nickname") ? json.getString("nickname") : json.getString("name"));
